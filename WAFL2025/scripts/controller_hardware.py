...
class SteeringController:
    def __init__(self, wheelbase=0.4, base_speed=1.0):
        ...
        self.spin_steer_angle = np.radians(25)   # ±25° works for many 4WS platforms
        ...
        if self.goal_pose is not None:
            ...
            if dist_to_goal < self.position_tolerance:
                ...
                if abs(heading_error_goal) < self.orientation_tolerance:
                    ...
                    rospy.loginfo_throttle(
                        1.0,
                        f"Pose fixing ! steer_rear={np.degrees(steer_rear):.1f}°, steer_front={np.degrees(steer_front):.1f}°")
